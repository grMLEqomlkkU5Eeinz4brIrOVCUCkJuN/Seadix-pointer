...
async function main(): Promise<void> {
	...
	// Config flags for comparison runs
	const allowWhitespace = String(process.env.ALLOW_WHITESPACE ?? "0") === "1";
	const noNormalization = String(process.env.NO_NORMALIZATION ?? "0") === "1";
	if (typeof trie.setAllowWhitespaceInTokens === "function") {
		trie.setAllowWhitespaceInTokens(allowWhitespace);
	}
	if (typeof trie.setNormalizationEnabled === "function") {
		trie.setNormalizationEnabled(!noNormalization);
	}
	const withGc = String(process.env.LOCK_GC ?? "1") === "1";
	const iters = Number(process.env.ITERATIONS ?? 3);

	const filePath = path.resolve(__dirname, "../textfiles/terms.txt");
	...
	console.log("\nRound 1/3: insertFromFile terms.txt" +
    ` (allowWhitespace=${allowWhitespace}, normalization=${!noNormalization})`);
	{
		const linesInFile = fs.readFileSync(filePath, "utf8").split("\n").length;
		const s = await measureFunctionWithMemory(() => {
			void trie.insertFromFile(filePath);
			trie.clear();
		}, { iterations: iters, lockGc: withGc, warmupIterations: 1 });
		console.log(formatSample("insertFromFile terms.txt", s));
	...
	{
		const linesInFile = fs.readFileSync(searchFilePath, "utf8").split("\n").length;
		const s = await measureFunctionWithMemory(() => {
	...
	{
		const linesInFile = fs.readFileSync(removeFilePath, "utf8").split("\n").length;
		const s = await measureFunctionWithMemory(() => {
